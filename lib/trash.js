...
export const reminderJobIsActive = (reminder, projects, pipelineEntries, parts = null) =>
  ...
  // Parts are only checked when the caller has them to check against, so
  // a page that doesn't load parts doesn't hide those reminders.
  (!reminder.partId || !parts || parts.some(x => x.id === reminder.partId));

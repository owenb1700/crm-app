...
// Trade words many different firms share -- a match on these alone
// ("Mechanical Services") doesn't suggest the same company.
const GENERIC_WORDS = new Set([
  "and", "of", "mechanical", "mechanicals", "service", "services", "engineering", "engineers", "engineer", "associates",
  "associated", "refrigeration", "refridgeration", "heating", "cooling", "air", "conditioning", "hvac", "plumbing",
  "electric", "electrical", "contractors", "contractor", "contracting", "construction", "builders", "building",
  "industries", "industrial", "systems", "system", "consultants", "consulting", "design", "designs", "solutions",
  "technologies", "technology", "enterprises", "partners", "commercial", "controls", "sheet", "metal", "fire",
  "protection", "energy", "environmental", "facilities", "facility", "management", "maintenance", "equipment",
  "supply", "sales", "north", "south", "east", "west", "northern", "southern", "midwest", "chicago", "illinois",
  "indiana", "wisconsin", "national", "american", "united", "general", "total", "quality", "professional", "pro"
]);
const distinctiveWords = (words) => words.filter(w => w.length >= 3 && !GENERIC_WORDS.has(w));
...
function similarNormalized(na, nb) {
  ...
  if (ta.length === tb.length) {
    ...
    if (differing.length === 1) {
      ...
      const shared = ta.filter(w => w !== x);
      const typo = Math.min(x.length, y.length) >= 5 && Math.abs(x.length - y.length) <= 2 && similarity(x, y) >= 0.8;
      if (typo && (distinctiveWords(shared).length >= 1 || (!GENERIC_WORDS.has(x) && !GENERIC_WORDS.has(y)))) return true;
    }
  } else if (
    Math.min(ca.length, cb.length) >= 8 &&
    ca.slice(0, 2) === cb.slice(0, 2) &&
    Math.abs(ca.length - cb.length) <= Math.max(ca.length, cb.length) * 0.1 &&
    similarity(ca, cb) >= 0.9
  ) {
    return true;
  }
  ...
  return short.length >= 2 && distinctiveWords(short).length >= 1 && long.length - short.length <= 2 && short.every(w => long.includes(w));
}

...
// The setup link sent when an admin creates an account expires 48 hours
// after it's sent. Every other link (an admin resend, or the person using
// "Forgot password?" themselves) never expires on its own -- it stays
// valid until a new password is actually set. `newAccount` only ever
// shortens a link, so it's harmless for the public route to accept it, and
// it's ignored if this uid has already been issued a link before.
// Requesting a new link always supersedes whatever's still outstanding, so
// only the most recently sent email can ever work.
export async function issueResetToken(db, { uid, email, newAccount = false }) {
  const col = db.collection(COLLECTION);

  const allSnap = await col.where("uid", "==", uid).get();
  const isFirstEver = newAccount && allSnap.empty;

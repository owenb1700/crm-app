...
// is matched to an existing company by normalized name -- filling in what the
// company is missing -- or created with its duplicate-guard key; each person
...
export async function importDirectoryFirms({ firms, importId, uid }) {
  ...
  for (const f of firms) {
    ...
    if (!company) {
      ...
      const patch = {
        category: f.category || company.category || "Contractor",
        address,
        phone,

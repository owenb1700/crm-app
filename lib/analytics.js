...
// Analytics is open to everyone with an account -- at minimum you see your
// own numbers.
export const canViewAnalytics = (profile) => !!profile && !profile.disabled;

// Whether the page shows the whole company (and can be filtered by
// salesperson) or only the viewer's own work. Granted per person in Admin
// Settings; on by default for Estimating and Admin (lib/permissions.js).
export const canViewOthersStats = (profile) =>
  !!profile && !profile.disabled &&
  effectivePermissions(profile.role, profile.permissions).viewOthersStats === true;
...
// Amounts are shown one way across the whole site, however they were
// typed: 4200 and 4200.00 and "4,200" all read as $4,200, and shorthand
// is spelled out -- 450k is $450,000, 1.2M is $1,200,000. Cents are kept
// only when there are any.
//
// Anything that isn't just an amount ("TBD", "5-10k", "per unit") is left
// exactly as written, since guessing at it would lose what someone meant.
...
// The stored form: tidy digits, no dollar sign (the boxes show their own).
export const normalizeMoneyText = (value) => {
  const text = String(value ?? "").trim();
  if (!text) return "";
  const amount = parseMoney(text);
  return amount === null ? text : formatAmount(amount);
};

// The displayed form: the same tidy digits with a dollar sign in front.
export const withDollar = (value) => {
  const text = String(value ?? "").trim();
  if (!text) return "";
  const amount = parseMoney(text);
...
// Projects (won work), the counterpart to summarize() for pipeline entries.
// Value comes from the project's own projectValue field.
export function summarizeProjects(projects) {
  const s = { count: 0, ongoing: 0, closed: 0, volume: 0, missingValue: 0 };
  (projects || []).forEach(p => {
    s.count += 1;
    if (p.category === "Project Closed") s.closed += 1;
    else s.ongoing += 1;
    const v = parseMoney(p.projectValue);
    if (v === null) s.missingValue += 1;
    else s.volume += v;
  });
  s.avgValue = s.count - s.missingValue > 0 ? s.volume / (s.count - s.missingValue) : null;
  return s;
}

// One row per key with both sides of the business on it: pipeline bids and
// projects. `keyOf` returns the grouping key for each record; `labelOf`
// turns that key into what's shown.
export function combinedBreakdown({ entries, projects, entryKeyOf, projectKeyOf, keys, labelOf = (k) => k }) {
  const rows = new Map();
  const row = (key) => {
    if (!rows.has(key)) rows.set(key, { key, label: labelOf(key), entries: [], projects: [] });
    return rows.get(key);
  };
  (keys || []).forEach(k => row(k));
  (entries || []).forEach(e => row(entryKeyOf(e) || "Not set").entries.push(e));
  (projects || []).forEach(p => row(projectKeyOf(p) || "Not set").projects.push(p));

  return [...rows.values()]
    .map(r => ({ key: r.key, label: r.label, pipeline: summarize(r.entries), projects: summarizeProjects(r.projects) }))
    .sort((a, b) =>
      (b.pipeline.volume.total + b.projects.volume) - (a.pipeline.volume.total + a.projects.volume) ||
      (b.pipeline.total + b.projects.count) - (a.pipeline.total + a.projects.count) ||
      String(a.label).localeCompare(String(b.label))
    );
}

// Counts and volume across both sides of the business at once: pipeline
// entries with their own outcome, and projects, which are won work by
// definition. Callers decide which records go in (see lib/analyticsFilters)
// so nothing is counted twice.
export function summarizeMixed({ entries = [], projects = [] }) {
  const rows = [
    ...entries.map(e => ({ status: statusOf(e), value: parseMoney(e.value) })),
    ...projects.map(p => ({ status: "won", value: parseMoney(p.projectValue) }))
  ];
  const s = {
    total: rows.length, won: 0, lost: 0, dnb: 0, open: 0,
    volume: { total: 0, won: 0, lost: 0, dnb: 0, open: 0 },
    missingValue: 0
  };
  rows.forEach(({ status, value }) => {
    s[status] += 1;
    if (value === null) {
      s.missingValue += 1;
    } else {
      s.volume.total += value;
      s.volume[status] += value;
    }
  });
  s.bidOn = s.total - s.dnb;
  s.decided = s.won + s.lost;
...
// Per-person numbers. A shared job is listed in full for everyone working
// it (counts stay whole numbers), while its value is divided by each
// person's share, so the money is never counted twice.
export function byPersonWeighted({ entries = [], projects = [], people, sharesOfEntry, sharesOfProject, labelOf }) {
  const rows = new Map();
  const row = (id) => {
    if (!rows.has(id)) {
      rows.set(id, {
        key: id,
        label: labelOf(id),
        pipeline: { total: 0, won: 0, lost: 0, dnb: 0, open: 0, volume: { total: 0, won: 0, lost: 0, dnb: 0, open: 0 } },
        projects: { count: 0, ongoing: 0, closed: 0, volume: 0 }
      });
    }
    return rows.get(id);
  };
  (people || []).forEach(id => row(id));

  entries.forEach(e => {
    const status = statusOf(e);
    const value = parseMoney(e.value) || 0;
    sharesOfEntry(e).forEach(({ userId, weight }) => {
      const r = row(userId).pipeline;
      r.total += 1;
      r[status] += 1;
      r.volume.total += value * weight;
      r.volume[status] += value * weight;
    });
  });

  projects.forEach(p => {
    const value = parseMoney(p.projectValue) || 0;
    sharesOfProject(p).forEach(({ userId, weight }) => {
      const r = row(userId).projects;
      r.count += 1;
      if (p.category === "Project Closed") r.closed += 1;
      else r.ongoing += 1;
      r.volume += value * weight;
    });
  });

  return [...rows.values()]
    .map(r => {
      const decided = r.pipeline.won + r.pipeline.lost;
      return {
        ...r,
        pipeline: { ...r.pipeline, decided, winRate: decided ? r.pipeline.won / decided : null }
      };
    })
    .sort((a, b) =>
      (b.pipeline.volume.total + b.projects.volume) - (a.pipeline.volume.total + a.projects.volume) ||

...
// Per-person numbers, where a shared job counts for each person by their
// share of the split (see lib/splits.js). Counts are fractional for shared
// jobs -- half a job each on a 50/50 split -- so nothing is counted twice.
export function byPersonWeighted({ entries = [], projects = [], people, sharesOfEntry, sharesOfProject, labelOf }) {
  ...
  projects.forEach(p => {
    ...
    sharesOfProject(p).forEach(({ userId, weight }) => {
      ...
      r.count += weight;
      if (p.category === "Project Closed") r.closed += weight;
      else r.ongoing += weight;

...
// Parts: they can be who the request came from, or a contact at one of
// the contractors doing the work.
export function partsForPerson(parts, person, firmName) {
  return (parts || [])
    .filter(p =>
      isThem(person, firmName, p.company, p.contact) ||
      (p.contractors || []).some(c => isThem(person, firmName, c.company, c.contact))
    )
    .map(p => ({
      kind: "Parts",
      id: p.id,
      ...
      address: p.projectAddress || "",
      role: isThem(person, firmName, p.company, p.contact) ? "Contact" : "Contractor contact"
    }));
}

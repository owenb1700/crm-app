...
import { isPipelineBidAlertFor, isWonFollowUpFor, isProjectCheckInFor, isPipelineCheckInFor } from "./alertRecipients";
...
export function buildDigestHtml({ customers: allCustomers, pipelineEntries: allPipeline, reminders: allReminders, uid, role, daysAhead, includeOverdue, intro }) {
  ...
  // Won entries waiting to be converted, and open entries with their own
  // next date (a Post-Bid entry coming back round a month later).
  const pipelineDue = pipelineEntries
    .filter(p => isWonFollowUpFor(p, uid, role) || isPipelineCheckInFor(p, uid, role))

...
import { isPipelineBidAlertFor } from "./directory";
...
export function buildDigestHtml({ customers, pipelineEntries, reminders, uid, role, daysAhead, includeOverdue, intro }) {
  const now = new Date();
  ...
  const reminderRow = (r) => row(
    ...
    escapeHtml(r.notes),
    r.date,
    r.pipelineId ? `${BASE_URL}/dashboard/pipeline/${r.pipelineId}` : `${BASE_URL}/dashboard`
  );

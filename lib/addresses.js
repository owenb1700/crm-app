...
//
// A building's sector is its own, set against the address (see
// lib/buildingSectors.js). It is never borrowed from the firms who worked
// there: their addresses are offices and shops, and their sectors describe
// them, not the building.
...
export const matchesAddressSearch = (building, q) => {
  ...
  return [
    ...(building.firms || []), ...(building.people || []), ...(building.sectors || [])
...
export const filterBuildings = (buildings, { search = "", sector = "", firm = "", kind = "" } = {}) =>
  (buildings || [])

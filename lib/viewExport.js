...
export async function exportDashboardView({
  ...
  filtered = false, jobNameOf = () => "", summary = []
}) {
  ...
  // Scorecard totals ride along as their own rows at the top, so the sheet
  // keeps one shape and still sorts and filters cleanly.
  const summaryRows = summary.map(t => {
    const cells = blanks(VIEW_HEADERS.length);
    cells[0] = "Summary";
    cells[1] = t.label;
    cells[7] = String(t.value ?? "");
    return cells;
  });
  ...
  await downloadTable({
    ...
    rows: [...summaryRows, ...items.map(i => i.cells)]
  });
  return items.length;
}

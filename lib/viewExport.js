...
export async function exportDashboardView({
  page, projects = [], pipelineEntries = [], reminders = [], users = [], viewer, format, filtered = false, jobNameOf = () => ""
}) {
  ...
  const base = page === "pipeline" ? "pipeline" : "my-projects";
  await downloadTable({
    format,
    ...
    sheetName: page === "pipeline" ? "Pipeline" : "My Projects",

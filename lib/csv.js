...
// Every export in the app goes through this, so CSV, Excel, and PDF always
...
export async function downloadTable({ filename, headers, rows, format = "csv", sheetName }) {
  ...
  if (format === "xlsx") {
    ...
    // Loaded only when someone actually exports a PDF, so the PDF library
    // isn't downloaded with every page.
    const { downloadPdf } = await import("./pdf");
    downloadPdf({ filename, title: name, headers, rows });
  } else {

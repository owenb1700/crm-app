...
const COLLECTION_OF = { project: "customers", pipeline: "pipeline", part: "parts" };

export async function loadRecord(kind, id) {
  const db = getAdminDb();
  const ref = db.collection(COLLECTION_OF[kind] || "pipeline").doc(id);
...
// A parts request and everything hanging off it: its notes, and anyone's
// reminders attached to it.
export async function deletePartCompletely(id) {
  const db = getAdminDb();
  const record = await loadRecord("part", id);
  if (!record) return null;

  const summary = { name: record.data.item || "Parts request" };
  summary.reminders = await deleteMatching(db.collection("reminders").where("partId", "==", id));
  await db.recursiveDelete(record.ref);
  return summary;
}

// Moves a project, pipeline entry or parts request to the Trash. Nothing attached to it is
...
export async function purgeExpiredTrash(now = new Date()) {
  ...
  const [projects, pipeline, parts] = await Promise.all([
  ...
  for (const d of parts.docs.filter(expired)) {
    await deletePartCompletely(d.id);
    purged.push({ kind: "part", id: d.id });
  }
  return purged;
}

...
// Everything a parts request can be found by, on the Parts tab and on the
// site-wide Search page alike: what was ordered, who bought it -- a
// contractor or a building owner, whoever the request came from -- anyone
// else named on the job, and where it shipped.
export const matchesPartSearch = (p, q) => {
  const needle = (q || "").trim().toLowerCase();
  if (!needle) return true;
  return [
    p.item, p.company, p.contact, p.email, p.phone, p.notes, p.stage, p.value, p.projectAddress,
    ...(p.contractors || []).flatMap(c => [c.company, c.contact])
  ]
    .filter(Boolean)
...
// Parts sales, for Analytics. Small jobs, but they're money, and the
// people doing them should see them counted. Requests with no usable
// value are counted but left out of the total, the same way projects are.
export function summarizeParts(parts) {
  const list = parts || [];
  const open = list.filter(p => p.stage !== "Closed" && p.stage !== "Invoiced");
  const values = list.map(partValue);
  const missingValue = list.filter(p => parseMoney(p.value) === null).length;
  const total = values.reduce((sum, v) => sum + v, 0);
  const counted = list.length - missingValue;
  return {
    count: list.length,
    open: open.length,
    done: list.length - open.length,
    volume: total,
    avgValue: counted ? total / counted : 0,
    missingValue
  };
}

// Credited to whoever entered the request.
export const partsByPerson = (parts, nameOf) => {
  const rows = new Map();
  (parts || []).forEach(p => {
    const id = p.ownerId || "";
    const row = rows.get(id) || { id, name: nameOf(id) || "Unassigned", count: 0, volume: 0 };
    row.count += 1;
    row.volume += partValue(p);
    rows.set(id, row);
  });
  return [...rows.values()].sort((a, b) => b.volume - a.volume || a.name.localeCompare(b.name));
};

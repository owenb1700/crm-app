...
const newestFirst = (rows) => (rows || []).filter(Boolean).sort((a, b) => dateOf(b).localeCompare(dateOf(a)));
...
export function buildingMemory({ address, projects = [], pipeline = [], parts = [] } = {}) {
  ...
  // Rows come straight from the database, and a blank or half-written one
  // must not take a form down with it.
  const here = (r) => Boolean(r) && sameAddress(r.projectAddress, address);
...
export function equipmentBySerial(serial, projects = []) {
  ...
  for (const job of newestFirst(projects)) {
    for (const row of equipmentRowsFrom(job) || []) {
      if (!row || clean(row.serial).toLowerCase() !== needle) continue;
...
export function modelMemory({ manufacturer, model }, towerModels = [], projects = []) {
  ...
  const record = (towerModels || []).filter(Boolean).find(t => clean(t.model).toLowerCase() === wanted);
  if (record) {
    return suggestion(
      {
  ...
  for (const job of newestFirst(projects)) {
    for (const row of equipmentRowsFrom(job) || []) {
      if (!row || clean(row.model).toLowerCase() !== wanted) continue;
...
export function salespersonForFirm(firmName, { projects = [], pipeline = [], nameOf = (id) => id } = {}) {
  ...
  const mine = [...(projects || []), ...(pipeline || [])].filter(r => r && sameCompany(r.company, firm) && clean(r.ownerId));
...
export function firmFromEmail(email, { contacts = [], companies = [] } = {}) {
  ...
  const byWebsite = (companies || []).filter(Boolean).find(c => clean(c.website).toLowerCase().includes(domain));
...
const findPersonNamed = (contacts, name) => {
  ...
  if (!needle) return null;
  return (contacts || []).filter(Boolean).find(c => clean(c.name).toLowerCase() === needle) || null;
};
